...
def call() {
    ...
    sh "cd jobs && python3.5 -m pipenv --python 3.6 install --skip-lock"
    ...
    withCredentials([file(credentialsId: 'charm_creds', variable: 'CHARMCREDS'),
        ...
        sh "cd jobs/infra && python3.5 -m pipenv run ansible-playbook playbook-jenkins.yml --extra-vars 'charm_creds=${CHARMCREDS} juju_creds=${JUJUCREDS} snapcraft_creds=${SNAPCRAFTCREDS}'"
    }
}
